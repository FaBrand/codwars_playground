...
bool is_register(std::string const& val){
  const auto result{std::find_if(val.begin(), val.end(),
                                 [](auto const& c){ return std::isalpha(c);})};
...
public:
  Mov(std::string param_1, std::string param_2):
  register_(param_1),
  value_(param_2){}
...
public:
  Jnz(std::string param_1, std::string param_2):
  register_(param_1),
  value_(param_2){}
...
void Jnz::operate_on(Machine& machine){
  if(ValueResolver::get_value_of(machine.get_registers(), register_) == 0)
    return;
...
InstructionFactory::InstructionFactory(){
  instruction_map_.emplace("mov", [](auto const& tokens){ return std::make_unique<Mov>(tokens.at(0), tokens.at(1)); });
  instruction_map_.emplace("jnz", [](auto const& tokens){ return std::make_unique<Jnz>(tokens.at(0), tokens.at(1)); });
...
Instruction_ptr InstructionFactory::create_instruction(std::string const& name, std::vector<std::string> const& arguments){
...
void Machine::load_program(RawProgram const& prog){
  for(auto const& instruction: prog){
    ...
    program_.push_back(instruction_factory_.create_instruction(tokens.front(), { std::next(tokens.begin(), 1), tokens.end()}));
  }
  ip_ = program_.begin();
}
